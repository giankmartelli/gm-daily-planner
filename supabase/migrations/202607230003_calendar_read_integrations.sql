...
create table if not exists public.calendar_event_links (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  external_event_id uuid not null references public.external_calendar_events(id) on delete cascade,
...
create policy external_calendars_owner_read on public.external_calendars for select to authenticated using (auth.uid() = user_id);
create policy external_events_owner_read on public.external_calendar_events for select to authenticated using (auth.uid() = user_id);
create policy event_links_owner_all on public.calendar_event_links for all to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
