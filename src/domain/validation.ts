...
export function sanitizeDay(value: unknown): DayData {
  ...
  const tasks = Array.isArray(raw.tasks) ? raw.tasks.flatMap((item) => {
    ...
    const score = (input: unknown, fallback: number) => Math.min(1, Math.max(0, Number.isFinite(Number(input)) ? Number(input) : fallback))
    return [{ ...normalized, category: CATEGORIES.includes(normalized.category) ? normalized.category : 'Otras', tags: normalized.tags.map((tag) => cleanText(tag, LIMITS.tag)).filter(Boolean).slice(0, LIMITS.tags), subtasks: normalized.subtasks.flatMap((subtask) => { const subTitle = cleanText(subtask.title, LIMITS.subtask); return subTitle ? [{ ...subtask, id: cleanText(subtask.id, 120) || crypto.randomUUID(), title: subTitle, completed: Boolean(subtask.completed) }] : [] }), estimatedMinutes: Math.min(1440, Math.max(5, Number(normalized.estimatedMinutes) || 30)), dueDate: /^\d{4}-\d{2}-\d{2}$/.test(normalized.dueDate ?? '') ? normalized.dueDate : undefined, energyLevel: energyLevels.includes(normalized.energyLevel ?? 'media') ? normalized.energyLevel : 'media', flexibility: flexibility.includes(normalized.flexibility ?? 'flexible') ? normalized.flexibility : 'flexible', preferredPeriod: periods.includes(normalized.preferredPeriod ?? 'cualquiera') ? normalized.preferredPeriod : 'cualquiera', trackedMinutes: Math.min(100000, Math.max(0, Number(normalized.trackedMinutes) || 0)), importance: score(normalized.importance, .5), urgency: score(normalized.urgency, .5), focusRequired: score(normalized.focusRequired, .5), completionScore: score(normalized.completionScore, 0), difficulty: score(normalized.difficulty, .5), context: cleanText(normalized.context, 80) || 'Otras', dependencies: (normalized.dependencies ?? []).map((id) => cleanText(id, 120)).filter(Boolean).slice(0, 50), preferredDays: (normalized.preferredDays ?? []).map(Number).filter((day) => Number.isInteger(day) && day >= 0 && day <= 6), calendarLocked: Boolean(normalized.calendarLocked), movable: normalized.movable !== false, createdAutomatically: Boolean(normalized.createdAutomatically), aiGenerated: Boolean(normalized.aiGenerated), interruptible: normalized.interruptible !== false, deepWork: Boolean(normalized.deepWork) }]
  }) : []
