...
export function CalendarConnections({user}:{user:User|null}){
  ...
  return <section className="panel calendar-connections"><header><div><span className="icon-box blue"><Cloud size={17}/></span><div><h2>Calendarios conectados</h2><p>Solo lectura · tus reuniones bloquean tiempo</p></div></div></header>
    {!user?<div className="calendar-connect-empty"><CalendarDays size={22}/><strong>Inicia sesión para conectar calendarios</strong><p>La agenda local seguirá funcionando sin conexión.</p></div>:<div className="calendar-provider-list">{(['google','outlook'] as const).map(provider=>{const connection=connections.find(item=>item.provider===provider&&item.status!=='disconnected');const label=provider==='google'?'Google Calendar':'Outlook Calendar';return <article key={provider}><div className={`provider-mark ${provider}`}>{provider==='google'?'G':'O'}</div><div><strong>{label}</strong><small>{connection?.email??'No conectado'}</small></div>{connection?<><SyncStatusBadge status={connection.status}/><button disabled={Boolean(busy)} onClick={()=>void run(provider,'sync')} aria-label={`Sincronizar ${label}`}><RefreshCw size={14}/></button><button disabled={Boolean(busy)} onClick={()=>void run(provider,'disconnect')} aria-label={`Desconectar ${label}`}><Unplug size={14}/></button></>:<button className="connect-calendar" disabled={Boolean(busy)} onClick={()=>void run(provider,'connect')}>Conectar</button>}</article>})}</div>}
