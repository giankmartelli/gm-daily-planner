...
export function AuthDialog({ open, onClose }: Props) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!supabase) return setStatus('Configura las variables de Supabase para activar la nube.')
  ...
  return <div className="modal-shade" onMouseDown={onClose}><section className="auth-dialog" role="dialog" aria-modal="true" aria-labelledby="auth-title" onMouseDown={(event) => event.stopPropagation()}><button className="auth-close" onClick={onClose} aria-label="Cerrar"><X size={18}/></button><span className="auth-icon"><Cloud size={24}/></span><h2 id="auth-title">{mode === 'login' ? 'Sincroniza tu espacio' : 'Crea tu cuenta'}</h2><p>{isSupabaseConfigured ? 'Accede desde cualquier dispositivo sin perder tu modo offline.' : 'El modo local sigue activo. Añade las credenciales de Supabase para habilitar autenticación.'}</p><form onSubmit={submit}><label>Correo electrónico<input type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} required/></label><label>Contraseña<input type="password" autoComplete={mode === 'login' ? 'current-password' : 'new-password'} minLength={8} value={password} onChange={(event) => setPassword(event.target.value)} required/></label>{status && <p className="auth-status" role="status">{status}</p>}<button className="auth-submit" disabled={loading || !isSupabaseConfigured}>{loading && <LoaderCircle className="spin" size={15}/>} {mode === 'login' ? 'Iniciar sesión' : 'Crear cuenta'}</button></form><button className="auth-switch" onClick={() => setMode(mode === 'login' ? 'signup' : 'login')}>{mode === 'login' ? '¿No tienes cuenta? Crear una' : 'Ya tengo cuenta'}</button></section></div>
}
